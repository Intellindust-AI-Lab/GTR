...
# ───────────── HA4DETR GPU batched Hungarian (optional, env GTR_GPU_LSAP=1) ─────────────
# Replaces the per-image scipy linear_sum_assignment loop with one GPU kernel launch.
# Only the *solver* changes — assignments minimize the same cost, so the loss is unchanged
# (ties may be broken differently, which is numerically equivalent). Any failure falls back
# to scipy automatically.
_HG_EXT = None
_HG_EXT_TRIED = False
_HG_MAX = 300        # extension compile-time MAX_ROWS == MAX_COLS
_HG_CLAMP = 1e15     # clamp real costs below the kernel's INF=1e20 padding sentinel


def _load_hungarian_ext():
    """Import HA4DETR's load_inline module exactly once; return hungarian_gpu or None."""
    global _HG_EXT, _HG_EXT_TRIED
    if _HG_EXT_TRIED:
        return _HG_EXT
    _HG_EXT_TRIED = True
    try:
        import importlib.util
        # Default to the vendored copy next to this file (stable); env override allowed.
        # Let torch auto-detect the GPU arch (verified to work on sm_80 without a forced flag).
        default_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hungarian_gpu_batch.py")
        path = os.environ.get("HA4DETR_PATH", default_path)
        spec = importlib.util.spec_from_file_location("hungarian_gpu_batch", path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)  # triggers JIT compile
        _HG_EXT = mod.hungarian_gpu
    except Exception as e:  # any failure -> scipy fallback
        print(f"[HungarianMatcher] GPU LSAP unavailable, using scipy: {e}")
        _HG_EXT = None
    return _HG_EXT


def _gpu_lsap(C_gpu, sizes, group_detr, g_num_queries, ext):
    """GPU drop-in for the per-image scipy loop. C_gpu: [bs, num_queries, total_gt] CUDA,
    BEFORE .cpu()/nan_to_num. Returns the same list[(index_i, index_j)] contract as scipy."""
    bs = C_gpu.shape[0]
    device = C_gpu.device
    INF = 1e20

    C_clean = torch.nan_to_num(C_gpu, nan=1.0, posinf=_HG_CLAMP, neginf=-_HG_CLAMP)
    C_clean = C_clean.clamp(-_HG_CLAMP, _HG_CLAMP)
    C_groups = C_clean.split(g_num_queries, dim=1)  # group_detr chunks [bs, g_num_queries, total_gt]

    B = group_detr * bs
    cost = torch.full((B, _HG_MAX, _HG_MAX), INF, dtype=torch.float32, device=device)
    ncols = torch.zeros(B, dtype=torch.int32, device=device)
    col_starts = [0]
    for n in sizes:
    ...
    for g_i, C_g in enumerate(C_groups):
        for i in range(bs):
            ...
            if Ni == 0:
                continue
            cs = col_starts[i]
            cost[b, :g_num_queries, :Ni] = C_g[i, :, cs:cs + Ni].to(torch.float32)

    out = ext(cost.contiguous(), ncols).cpu()  # [B, 300, 2] int32; tiny host transfer
    ...
    for g_i in range(group_detr):
        ...
        for i in range(bs):
            o = out[g_i * bs + i]
            col = o[:g_num_queries, 1]          # assigned GT-col per query-row (-1 if unmatched)
            mask = col >= 0
            rows = torch.nonzero(mask, as_tuple=False).squeeze(1)  # ascending == scipy row_ind
            index_i = (rows + g_i * g_num_queries).to(torch.int64)
            index_j = col[mask].to(torch.int64)
            indices_g.append((index_i, index_j))
        if indices is None:
            indices = indices_g
        else:
            indices = [(torch.cat([a[0], b[0]]), torch.cat([a[1], b[1]]))
...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, weight_dict, use_focal_loss=False, alpha=0.25, gamma=2.0,
        ...
        # HA4DETR GPU Hungarian: optional. For GTR's small per-rank batch
        # (bs=4 at 32 GPUs), scipy is faster in real-loop probes.
        # Falls back to scipy automatically if the CUDA ext can't build/load.
        self.use_gpu_lsap = os.environ.get("GTR_GPU_LSAP", "0") == "1"
    ...
    @torch.no_grad()
    def forward(self, outputs: Dict[str, torch.Tensor], targets, return_topk=False, group_detr=1):
        ...
        masks_present = (
            ...
            and len(targets) > 0 and 'masks' in targets[0]
        )
        if masks_present:
            ...
            pred_masks_logits = point_sample(out_masks.unsqueeze(1),
                                             point_coords.repeat(out_masks.shape[0], 1, 1),
                                             align_corners=False).squeeze(1)
            tgt_masks_flat = point_sample(tgt_masks.unsqueeze(1),
        ...
        # GPU Hungarian (HA4DETR) fast path: keeps C on device (skips the host transfer),
        # one kernel launch instead of group_detr*bs scipy solves. Requires #queries-per-group
        # >= #gt-per-image; otherwise (and for return_topk / unavailable ext) fall back to scipy.
        ext = _load_hungarian_ext() if (self.use_gpu_lsap and not return_topk) else None
        if ext is not None and all(n <= g_num_queries for n in sizes):
            indices = _gpu_lsap(C.view(bs, num_queries, -1), sizes, group_detr, g_num_queries, ext)
        ...
        if return_topk:
            return {
                "indices_o2m": self.get_top_k_matches(
                    C, sizes=sizes, k=return_topk, initial_indices=
