...
def load_config(file_path, cfg=None):
    ...
    `cfg` defaults to None, not {}: a mutable default would be created once at
    def-time and shared by every call, so the second config loaded in a process
    would inherit the first one's keys (an earlier gtr_x leaking `ffn_ratio: 6`
    into a later gtr_s, an OBB config leaking `flatcosine`/`flat_epoch` into a
    pose one). Recursive calls still thread their own dict through explicitly.
    """
