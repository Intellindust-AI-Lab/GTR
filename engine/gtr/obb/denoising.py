...
def get_obb_contrastive_denoising_training_group(targets,
                                                 num_classes,
                                                 num_queries,
                                                 class_embed,
                                                 num_denoising=100,
    ...
    dn_positive_idx = torch.split(dn_positive_idx, [n * total_num_group for n in num_gts])
    ...
    for g in range(group_detr):
        ...
        if g < group_detr - 1:
            attn_mask[base:base + num_dn_per_g, (g + 1) * num_dn_per_g:num_denoising] = True
        ...
        for i in range(num_group):
            ...
            if i == 0:
                attn_mask[row_start:row_end, base + max_gt_num * 2 * (i + 1): base + num_dn_per_g] = True
            if i == num_group - 1:
