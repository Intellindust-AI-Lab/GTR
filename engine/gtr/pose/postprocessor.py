...
__all__ = ['GTRPosePostProcessor']

@register()
class GTRPosePostProcessor(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        if self.deploy_mode:
            # expand(-1, -1, ...) keeps the real batch and query dims; a literal 1 in the
            # batch slot only ever worked for single-image inference.
            keypoints = torch.gather(out_keypoints, 1, topk_keypoints[..., None, None].expand(-1, -1, self.num_body_points, 2))
