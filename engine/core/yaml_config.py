...
from ..misc.dist_utils import _seed_worker
...
class YAMLConfig(BaseConfig):
    ...
    def evaluator(self, ):
        if self._evaluator is None and 'evaluator' in self.yaml_cfg:
            ...
            elif self.yaml_cfg['evaluator']['type'] == 'DepthEvaluator':
                self._evaluator = create('evaluator', self.global_cfg)
            elif self.yaml_cfg['evaluator']['type'] == 'SemSegEvaluator':
                # GT comes from the val targets per batch; no dataset-level api needed.
                self._evaluator = create('evaluator', self.global_cfg)
            else:
    ...
    def get_rank_batch_size(cfg):
        ...
        if total_batch_size is None:
            ...
            assert total_batch_size % world_size == 0, \
                f'total_batch_size={total_batch_size} should be divisible by world_size={world_size}'
    ...
    def build_dataloader(self, name: str):
        ...
        # pop like total_batch_size: consumed by dist_utils.warp_loader, not DataLoader.__init__
        epoch_chunks = global_cfg[name].pop('epoch_chunks', 1)
        ...
        loader = create(
            ...
            persistent_workers=self.yaml_cfg[name].get('num_workers', self.num_workers) > 0,
    ...
    def reset_tranforms_size(self):
        """reset tranforms size according to input size
        """
        ...
        for ops in [train_ops, val_ops]:
            for op in ops:
                ...
                elif op["type"] == 'LargeScaleJitter':
                    op['output_size'] = (input_size, input_size)
