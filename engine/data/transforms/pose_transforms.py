...
---------------------------------------------------------------------------------
Pose-task data pipeline ported from DETRPose
(https://github.com/SebastianJanampa/DETRPose). These transforms operate on plain (PIL image, dict target) pairs where
target keeps pixel-space 'boxes' (xyxy) / 'keypoints' (N, K, 3) until PoseNormalize converts
them to the normalized cxcywh / concatenated [Z|V] format consumed by DETRPose.

Classes are prefixed with `Pose` where the original DETRPose name would collide with the
torchvision-based detection transforms already registered in this project.
"""
...
@register()
class PoseRandomZoomOut(object):
...
@register()
class PoseRandomHorizontalFlip(object):
...
class MixUp(object):
    ...
    def _resize_target(self, target: dict, scale_x: float, scale_y: float) -> dict:
        ...
        # `area` is the OKS scale denominator, so it has to follow the boxes -- same rule
        # as resize() above, which scales it by ratio_width * ratio_height.
        if 'area' in target:
    ...
    def __call__(self, img, target):
        ...
        for item in selected:
            ...
            if patch.size[0] == 0 or patch.size[1] == 0:
                continue
            # A patch larger than the canvas gets silently cropped by PIL.paste while the
            # box/keypoints below are written uncropped -> out-of-image annotations. Skip;
            # with sw <= width and sh <= height every pasted coordinate stays in-image.
            ...
            augmented_target['area'] = torch.cat([
                ...
                # The patch is pasted at its source size, so the source segmentation area
                # carries over. sw * sh is the bounding box instead, ~1/0.53 too large for
                # a person, which would shrink the OKS distance of every pasted instance.
...
class MixUpCopyPaste(object):
    def __init__(self, mixup_prob=0.5, copypaste_prob=1, max_cached_images=50, random_pop=True):
        ...
        self.mixup = MixUp(p=1.0, max_cached_images=max_cached_images, random_pop=random_pop)
    ...
    def __call__(self, img, target):
        # Both sub-transforms run with p=1, so the single roll below is a categorical split:
        # [0, mixup_prob) -> mixup, [mixup_prob, copypaste_prob) -> copypaste, rest unchanged.
        # (0.5, 1.0) gives the 50/50 recipe; (0.25, 0.75) reproduces the DETRPose reference,
        # whose internal re-rolls made the actual rates mixup 25% / copypaste 50% / none 25%.
        r = random.random()
...
@register()
class PoseCompose(T.Compose):
    """Compose for the DETRPose-style (image, target) transforms with the DETRPose stop_epoch
    policy: policy.epoch = [warmup_end, mosaic_end, strong_aug_end]. Strong augs (policy.ops)
    run only in [warmup_end, strong_aug_end); PoseMosaic additionally only in
    [warmup_end, mosaic_end) with probability mosaic_prob, mutually exclusive with
    PoseRandomZoomOut on the same sample.

    Exposes `stop_epoch` / `mosaic_epoch` scalars for DetSolver (no-aug boundary bookkeeping).
    """
    ...
    def __init__(self, ops, policy=None, mosaic_prob=-0.1) -> None:
        ...
        # Scalars read by DetSolver: aug-stop boundary and mosaic window end.
        if isinstance(policy.get('epoch'), list) and len(policy['epoch']) == 3:
            self.mosaic_epoch = policy['epoch'][1]
            self.stop_epoch = policy['epoch'][-1]
        else:
            self.mosaic_epoch = -1
            self.stop_epoch = 100000000

    def state_dict(self):
        return {'global_samples': self.global_samples}

    def load_state_dict(self, state):
        if not state:
            return
        self.global_samples = state.get('global_samples', self.global_samples)
            ...
            for transform in self.transforms:
                if (type(transform).__name__ in policy_ops and cur_epoch < policy_epoch[0]):
                    ...
                    if (type(transform).__name__ == 'PoseMosaic' and not with_mosaic):
                        pass
                    elif type(transform).__name__ == 'PoseRandomZoomOut' and with_mosaic:
                        pass
                    else:
