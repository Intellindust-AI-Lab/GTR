...
class CocoDetection(torchvision.datasets.CocoDetection, DetDataset):
    ...
    def __init__(self, img_folder, ann_file, transforms, return_masks=False, remap_mscoco_category=False):
        ...
        if return_masks:
            self._sanitize_empty_segmentations()

    def _sanitize_empty_segmentations(self):
        anns = self.coco.dataset.get('annotations', [])
        kept = [a for a in anns if not _is_segmentation_empty(a.get('segmentation'))]
        dropped = len(anns) - len(kept)
        if dropped == 0:
            return
        self.coco.dataset['annotations'] = kept
        self.coco.createIndex()
        print(f'CocoDetection: dropped {dropped} anns with empty segmentation from {self.ann_file}')
...
def convert_coco_poly_to_mask(segmentations, height, width):
    ...
    for segmentation in segmentations:
        if isinstance(segmentation, dict):
            # RLE segmentation: uncompressed (counts is a list) needs frPyObjects first.
...
def _is_segmentation_empty(segmentation):
    if segmentation is None:
        return True
    if isinstance(segmentation, dict):
        counts = segmentation.get("counts")
        return counts in (None, [], "")
    if isinstance(segmentation, list):
        if len(segmentation) == 0:
            return True
        return all(not isinstance(p, (list, tuple)) or len(p) < 6 for p in segmentation)
    return True


def has_valid_segmentation(segmentation, height, width):
    if not segmentation:
        return False
    if isinstance(segmentation, dict):
        size = segmentation.get("size")
        counts = segmentation.get("counts")
        if not size or list(size) != [height, width]:
            return False
        if counts in (None, [], ""):
            return False
        return True
    return any(len(poly) >= 6 for poly in segmentation)
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image: Image.Image, target, **kwargs):
        ...
        if self.return_masks:
            anno = [
                obj for obj in anno
                if has_valid_segmentation(obj.get("segmentation"), h, w)
            ]
