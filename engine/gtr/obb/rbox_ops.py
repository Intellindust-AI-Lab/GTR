...
def distance2rbox(ref_rbox: torch.Tensor, distance: torch.Tensor, reg_scale) -> torch.Tensor:
    """
...
def rbox2distance(ref_rbox: torch.Tensor, gt_rbox: torch.Tensor, reg_max, reg_scale, up, eps_clip=0.1):
    """
...
def _kld_distance(xy_p, sigma_p, xy_t, sigma_t, alpha=1.0):
    ...
    dx = xy_p[..., 0] - xy_t[..., 0]
    dy = xy_p[..., 1] - xy_t[..., 1]
    xy_distance = 0.5 * (inv_p_00 * dx * dx + 2 * inv_p_01 * dx * dy + inv_p_11 * dy * dy)
    # 0.5 * trace(Sigma_p^-1 Sigma_t)
    whr_distance = 0.5 * (inv_p_00 * sigma_t[..., 0, 0] + 2 * inv_p_01 * sigma_t[..., 0, 1]
...
def kld_cost_pairwise(pred: torch.Tensor, target: torch.Tensor, tau=1.0) -> torch.Tensor:
    """Pairwise KLD cost: (N,5)x(M,5) -> (N,M) in [0,1)."""
    xy_p, sigma_p = xy_wh_r_2_xy_sigma(pred)
    xy_t, sigma_t = xy_wh_r_2_xy_sigma(target)
    distance = _kld_distance(
        xy_p[:, None], sigma_p[:, None], xy_t[None, :], sigma_t[None, :]).clamp(min=1e-7)
    return 1 - 1 / (tau + torch.log1p(distance))
...
def chamfer_cost_pairwise(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    ...
    dist = torch.norm(c1[:, None, :, None, :] - c2[None, :, None, :, :], dim=-1)  # (N,M,4,4)
