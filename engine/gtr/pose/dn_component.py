...
def prepare_for_cdn(dn_args, training, num_queries, num_classes, num_keypoints, hidden_dim, label_enc, pose_enc, img_dim, device):
    ...
    if training:
        ...
        # keypoint noise
        # t['boxes'] is normalized cxcywh (PoseNormalize), so the human center is boxes[:, :2].
        # The DETRPose original applied the xyxy midpoint formula to these cxcywh boxes,
        # anchoring DN queries at the shifted point ((cx+w)/2, (cy+h)/2); fixed here on purpose.
        ...
        # F.normalize's second positional argument is `p`, not `dim`: passing -1 there set
        # p=-1 and left dim at its default 1, so the (x, y) noise direction had norm ~70
        # instead of 1 and pushed most DN keypoints far outside [0, 1].
