...
class ModelEMA(object):
    ...
    def __init__(self, model: nn.Module, decay: float=0.9999, warmups: int=1000, start: int=0):
        ...
        self.module = deepcopy(dist_utils.de_model(model)).eval()
    ...
    def update(self, model: nn.Module):
        ...
        with torch.no_grad():
            ...
            msd = dist_utils.de_model(model).state_dict()
            ema_tensors = []
            model_tensors = []
            for k, v in self.module.state_dict().items():
                if v.dtype.is_floating_point:
                    ema_tensors.append(v)
                    model_tensors.append(msd[k].detach())

            try:
                torch._foreach_mul_(ema_tensors, d)
                torch._foreach_add_(ema_tensors, model_tensors, alpha=1 - d)
            except (RuntimeError, TypeError):
                for v, src in zip(ema_tensors, model_tensors):
                    v *= d
                    v += (1 - d) * src
