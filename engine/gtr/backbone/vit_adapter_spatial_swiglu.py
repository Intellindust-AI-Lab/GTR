...
from .vit_adapter import ViTAdapter, VisionTransformer

__all__ = ['ViTAdapterSpatialSwiGLU', ]
...
class SpatialSwiGLU(nn.Module):
    ...
    def __init__(self, hidden_size, hidden_ratio=4, intermediate_size=None):
        ...
        self.dwconv = nn.Conv2d(intermediate_size, intermediate_size, 3, padding=1,
    ...
    def convert_to_deploy(self):
        ...
        del self.gate_proj
        self.dwconv.weight.data = self.dwconv.weight.data.contiguous(memory_format=torch.channels_last)
    ...
    def forward(self, x):
        if getattr(self, '_deploy_split_gemm', False):
            ...
            y = y.view(B, gs, gs, -1).permute(0, 3, 1, 2)     # channels_last view of [B, C, gs, gs]
...
class VisionTransformerSpatialSwiGLU(VisionTransformer):
    ...
    def __init__(self, mlp_ratio=4., **kwargs):
        ...
        for blk in self._model.blocks:
            blk.mlp = SpatialSwiGLU(hidden_size=self.embed_dim, hidden_ratio=int(mlp_ratio))
    ...
    def forward(self, x, return_layer_indexes=None, n=1, mask_ratio=0.0, num_stages=0):
        ...
        assert num_stages == 0, 'VisionTransformerSpatialSwiGLU has no CLS token; distillation path unsupported'
