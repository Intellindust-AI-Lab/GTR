...
class MSDeformableAttention(nn.Module):
    ...
    def convert_to_deploy(self):
        ...
        if getattr(self, '_fused_offset_weight', False):
            return
        ow = nn.Linear(self.embed_dim, self.total_points * 3,
                       device=self.sampling_offsets.weight.device,
                       dtype=self.sampling_offsets.weight.dtype)
        with torch.no_grad():
            ow.weight.copy_(torch.cat([self.sampling_offsets.weight,
                                       self.attention_weights.weight], dim=0))
            ow.bias.copy_(torch.cat([self.sampling_offsets.bias,
                                     self.attention_weights.bias], dim=0))
    ...
    def forward(self,
        ...
        if (getattr(self, '_atlas_mode', False) and not self.training
                and isinstance(value, tuple) and len(value) == 4 and value[1].dim() == 2
            ...
            sampled = F.grid_sample(canvas, g, mode='bilinear',
                                    padding_mode='zeros', align_corners=False)
            attn = attention_weights.permute(0, 2, 1, 3).reshape(
                bs * self.num_heads, 1, Len_q, sum(self.num_points_list))
            out = (sampled * attn).sum(-1).reshape(bs, self.num_heads * self.head_dim, Len_q)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        if pos_qk is not None and attn_mask is None and not self.training:
            ...
            return self._forward_post_self_attn(target, reference_points, value,
                                                spatial_shapes, query_pos_embed)
    ...
    def _forward_post_self_attn(self, target, reference_points, value,
                                spatial_shapes, query_pos_embed):
        # cross attention
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, hidden_dim, decoder_layer, decoder_layer_wide, num_layers, num_head, reg_max, reg_scale, up,
                 eval_idx=-1, layer_scale=2, act='relu', segmentation_head=None):
    ...
    def build_value_atlas(self, spatial_shapes, num_points_list, batch_size=1,
                          gutter=6, margin=3):
    ...
    def convert_to_deploy(self):
        ...
        d = self.hidden_dim
        wqk = torch.cat([l.self_attn.in_proj_weight[:2 * d] for l in self.layers], dim=0)
    ...
    def forward(self,
        ...
        selective_amp = self.training and amp_safe_mode in {'selective', 'heads_fp32', 'matcher_fp32'}
        device_type = memory.device.type
        output = target
        output_detach = pred_corners_undetach = 0
        value = self.value_op(memory, None, None, memory_mask, spatial_shapes, value_atlas)
        ...
        num_dn = dn_meta['dn_num_split'][0] if (dn_meta is not None and self.training) else 0
            ...
            if selective_amp:
                with torch.autocast(device_type=device_type, enabled=False):
                    ...
                    if i == 0 :
                        ...
                        pre_bboxes = F.sigmoid(pre_bbox_head(output_for_head) + inverse_sigmoid(ref_points_detach))
                        pre_scores = score_head[0](output_for_head)
                        ref_points_initial = pre_bboxes.detach()

                    pred_corners = bbox_head[i](output_for_head + output_detach_for_head) + pred_corners_prev
        ...
        return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), \
               ...
               dec_out_hidden, dec_out_masks, pre_segs
...
class GTRTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if eval_output_layer is None:
            ...
            if eval_output_layer < 0 or eval_output_layer > num_layers:
                raise ValueError(f'eval_output_layer must be in [0, {num_layers}], got {eval_output_layer}')
        ...
        if self.eval_idx < 0 or self.eval_idx >= num_layers:
            raise ValueError(f'eval_idx resolves to {self.eval_idx}, expected [0, {num_layers - 1}]')
        ...
        self.up = nn.Parameter(torch.tensor([0.5], dtype=torch.float32), requires_grad=False)
        self.reg_scale = nn.Parameter(torch.tensor([float(reg_scale)], dtype=torch.float32), requires_grad=False)
        decoder_layer = TransformerDecoderLayer(hidden_dim, nhead, dim_feedforward, dropout, \
            activation, num_levels, num_points, cross_attn_method=cross_attn_method, group_detr=group_detr)
        decoder_layer_wide = TransformerDecoderLayer(hidden_dim, nhead, dim_feedforward, dropout, \
            activation, num_levels, num_points, cross_attn_method=cross_attn_method, layer_scale=layer_scale, group_detr=group_detr)
        ...
        segmentation_head = SegmentationHead(hidden_dim, num_layers,
                                             downsample_ratio=mask_downsample_ratio,
                                             image_size=eval_spatial_size) if mask_downsample_ratio else None
        self.decoder = TransformerDecoder(hidden_dim, decoder_layer, decoder_layer_wide, num_layers, nhead,
                                          reg_max, self.reg_scale, self.up, self.eval_idx, layer_scale, act=activation,
      ...
      # denoising
        ...
        if group_detr > 1:
            self.enc_score_head_group = nn.ModuleList([copy.deepcopy(self.enc_score_head) for _ in range(group_detr)])
            self.enc_bbox_head_group = nn.ModuleList([copy.deepcopy(self.enc_bbox_head) for _ in range(group_detr)])
    ...
    def convert_to_deploy(self):
        ...
        if (self.eval_spatial_size is not None and self.cross_attn_method == 'default'
            ...
            assert all(l.cross_attn.num_points_list == num_points_list for l in self.decoder.layers)
    ...
    def _get_decoder_input(self,
        ...
        selective_amp = self.training and os.getenv('GTR_AMP_SAFE_MODE', '').lower() in {
            'selective', 'heads_fp32', 'matcher_fp32'
        }
            ...
            for g_idx in range(self.group_detr):
                if selective_amp:
                    with torch.autocast(device_type=memory.device.type, enabled=False):
                        enc_outputs_logits_g = self.enc_score_head_group[g_idx](memory_for_select)
                        enc_topk_memory_g, enc_topk_logits_g, enc_topk_anchors_g = \
                            self._select_topk(memory_for_select, enc_outputs_logits_g, anchors_for_select, self.num_queries)
                        enc_topk_bbox_unact_g = self.enc_bbox_head_group[g_idx](enc_topk_memory_g) + enc_topk_anchors_g
                else:
                    enc_outputs_logits_g = self.enc_score_head_group[g_idx](memory_masked)
                    enc_topk_memory_g, enc_topk_logits_g, enc_topk_anchors_g = \
                        self._select_topk(memory_masked, enc_outputs_logits_g, anchors, self.num_queries)
                    enc_topk_bbox_unact_g = self.enc_bbox_head_group[g_idx](enc_topk_memory_g) + enc_topk_anchors_g
                ...
                if self.learn_query_content:
                    content_g = self.tgt_embed.weight.unsqueeze(0).tile([memory.shape[0], 1, 1])
                else:
            ...
            if selective_amp:
                with torch.autocast(device_type=memory.device.type, enabled=False):
                    ...
                    enc_topk_memory, enc_topk_logits, enc_topk_anchors = \
                        self._select_topk(memory_for_select, enc_outputs_logits, anchors_for_select, self.num_queries)
                    enc_topk_bbox_unact: torch.Tensor = enc_bbox_head(enc_topk_memory) + enc_topk_anchors
            else:
                enc_outputs_logits: torch.Tensor = enc_score_head(memory_masked)
                enc_topk_memory, enc_topk_logits, enc_topk_anchors = \
                    self._select_topk(memory_masked, enc_outputs_logits, anchors, self.num_queries)
                enc_topk_bbox_unact: torch.Tensor = enc_bbox_head(enc_topk_memory) + enc_topk_anchors
        ...
        return content, enc_topk_bbox_unact, enc_topk_bboxes_list, enc_topk_logits_list, enc_topk_memory_list
    ...
    def _build_value_atlas(self, feats):
        ...
        for f, (h, w), (r0, c0) in zip(feats, dec._va_shapes, dec._va_origins):
            canvas[:, :, r0:r0 + h, c0:c0 + w] = f.view(bs, nh, c, h, w).flatten(0, 1)
        return canvas

    def forward(self, feats, targets=None, spatial_feat=None):
        value_atlas = None
        if (getattr(self.decoder, '_va_ready', False) and not self.training
                and len(feats) == len(self.input_proj)
                and tuple((f.shape[2], f.shape[3]) for f in feats) == self.decoder._va_shapes):
        ...
        init_ref_contents, init_ref_points_unact, enc_topk_bboxes_list, enc_topk_logits_list, enc_topk_memory_list = \
            self._get_decoder_input(memory, spatial_shapes, denoising_logits, denoising_bbox_unact)

        # decoder
        out_bboxes, out_logits, out_corners, out_refs, pre_bboxes, pre_logits, dec_hidden, out_masks, pre_segs = self.decoder(
            init_ref_contents,
    ...
    def _set_aux_loss2(self, outputs_class, outputs_coord, outputs_corners, outputs_ref,
                       outputs_masks=None, teacher_corners=None, teacher_logits=None):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        if outputs_masks is None:
            ...
            res = zip(outputs_class, outputs_coord, outputs_corners, outputs_ref, outputs_masks)
