...
from ..det.decoder import MLP, Gate
from .denoising import get_obb_contrastive_denoising_training_group
from .rbox_ops import PI, distance2rbox
from ..utils import (bias_init_with_prob, deformable_attention_core_func_v2,
                    get_activation, inverse_sigmoid, weighting_function)
...
class OBBMSDeformableAttention(nn.Module):
    ...
    def forward(self,
        ...
        if self.orthogonal_attn:
            ...
            head_phase = torch.zeros(self.num_heads, dtype=theta.dtype, device=theta.device)
            head_phase[self.num_heads // 2:] = math.pi / 2
            theta = theta + head_phase.view(1, 1, self.num_heads, 1, 1)  # [bs, Len_q, H, n_ref, 1]
...
class OBBTransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        self.cross_attn = OBBMSDeformableAttention(d_model, n_head, n_levels, n_points,
            method=cross_attn_method, orthogonal_attn=orthogonal_attn)
...
class OBBTransformerDecoder(nn.Module):
    ...
    def __init__(self, hidden_dim, decoder_layer, decoder_layer_wide, num_layers, num_head, reg_max, reg_scale, up,
                 eval_idx=-1, layer_scale=2, act='relu', decouple_angle_pe=False,
                 decouple_angle_refine=False, angle_decay_alpha0=1.5):
        super().__init__()
        ...
        self.lqe_layers = nn.ModuleList([copy.deepcopy(OBBLQE(4, 64, 2, reg_max, act=act)) for _ in range(num_layers)])
    ...
    def value_op(self, memory, value_proj, value_scale, memory_mask, memory_spatial_shapes):
        ...
        return tuple(
            ...
            for v, (h, w) in zip(value.split(split_shape, dim=-1), memory_spatial_shapes)
        )
    ...
    def convert_to_deploy(self):
        ...
        self.register_buffer('project', weighting_function(self.reg_max, self.up, self.reg_scale, deploy=True),
                             persistent=False)
    ...
    def forward(self,
        ...
        ref_for_pe = ref_points_detach[..., :4] if self.decouple_angle_pe else ref_points_detach
        ...
        for i, layer in enumerate(self.layers):
            ...
            output = layer(output, ref_points_input, value, spatial_shapes, attn_mask, query_pos_embed, num_dn=num_dn)
            ...
            inter_ref_bbox = distance2rbox(ref_points_initial, integral(pred_corners, project), reg_scale)
            ...
            if self.decouple_angle_refine:
                ...
                dtheta = torch.tanh(angle_head[i](output + output_detach)) * (self.angle_decay_alpha0 ** (-i))
                theta_new_rad = torch.remainder(theta_ref_rad + dtheta, PI)
                inter_ref_bbox = torch.cat([inter_ref_bbox[..., :4], theta_new_rad / PI], dim=-1)
...
class OBBGTRTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        decoder_layer = OBBTransformerDecoderLayer(hidden_dim, nhead, dim_feedforward, dropout,
        ...
        decoder_layer_wide = OBBTransformerDecoderLayer(hidden_dim, nhead, dim_feedforward, dropout,
            activation, num_levels, num_points, cross_attn_method=cross_attn_method, layer_scale=layer_scale, group_detr=group_detr,
            orthogonal_attn=orthogonal_attn)
        self.decoder = OBBTransformerDecoder(hidden_dim, decoder_layer, decoder_layer_wide, num_layers, nhead,
        ...
        self.query_pos_head = MLP(4 if decouple_angle_pe else 5, hidden_dim, hidden_dim, 3, act=activation)
        ...
        dec_bbox_head = MLP(hidden_dim, hidden_dim, NUM_DIST * (self.reg_max + 1), 3, act=activation)
        self.dec_bbox_head = nn.ModuleList(
            [dec_bbox_head if share_bbox_head else copy.deepcopy(dec_bbox_head) for _ in range(self.eval_idx + 1)]
          + [MLP(scaled_dim, scaled_dim, NUM_DIST * (self.reg_max + 1), 3, act=activation) for _ in range(num_layers - self.eval_idx - 1)])
        ...
        if decouple_angle_refine:
            self.angle_head = nn.ModuleList(
                [MLP(hidden_dim, hidden_dim, 1, 3, act=activation) for _ in range(self.eval_idx + 1)]
              + [MLP(scaled_dim, scaled_dim, 1, 3, act=activation) for _ in range(num_layers - self.eval_idx - 1)])
    ...
    def convert_to_deploy(self):
        ...
        if self.angle_head is not None:
            self.angle_head = nn.ModuleList(
                [self.angle_head[i] if i <= self.eval_idx else nn.Identity() for i in range(len(self.angle_head))]
            )
    ...
    def forward(self, feats, targets=None):
        ...
        if self.training and self.num_denoising > 0:
            denoising_logits, denoising_bbox_unact, attn_mask, dn_meta = \
                get_obb_contrastive_denoising_training_group(targets,
                    self.num_classes,
                    self.num_queries,
