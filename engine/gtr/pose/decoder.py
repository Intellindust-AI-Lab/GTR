...
from .dn_component import prepare_for_cdn
from .ms_deform_attn import MSDeformAttn
from .utils import MLP, _get_activation_fn, inverse_sigmoid

__all__ = ['GTRPoseTransformer']
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def with_pos_embed(tensor, pos, training):
        # Out-of-place equivalent of the DETRPose `tensor[:, :, -np:] += pos`: the original
        # relies on the in-place side effect (callers keep using the pos-added tensor), which
        # breaks autograd in pure FP32. Callers re-assign the returned tensor instead.
        if pos is not None:
            np = pos.shape[2]
    ...
    def forward(self,
        ...
        # within-instance self-attention
        tgt_pose = self.with_pos_embed(tgt_pose, tgt_pose_query_pos, self.training)
        q = k = tgt_pose.flatten(0, 1) # bs * nq, num_kpts, 2
        ...
        # deformable cross-attention
        tgt_pose = self.with_pos_embed(tgt_pose, tgt_pose_query_pos, self.training)
        tgt2_pose = self.cross_attn(tgt_pose.flatten(1, 2),
...
@register()
class GTRPoseTransformer(nn.Module):
