...
class DataLoader(data.DataLoader):
    ...
    def state_dict(self):
        ...
        if hasattr(self.collate_fn, 'state_dict'):
            state['collate_fn'] = self.collate_fn.state_dict()
        return state
    ...
    def load_state_dict(self, state):
        ...
        if 'generator' in state and getattr(self, 'generator', None) is not None:
            self.generator.set_state(state['generator'])
        if 'dataset' in state and hasattr(self.dataset, 'load_state_dict'):
            self.dataset.load_state_dict(state['dataset'])
        if 'collate_fn' in state and hasattr(self.collate_fn, 'load_state_dict'):
            self.collate_fn.load_state_dict(state['collate_fn'])
...
class BaseCollateFunction(object):
    def _ensure_epoch_state(self):
        ...
        if state is None:
            # collate_fn also runs inside DataLoader workers when num_workers > 0
...
@register()
class SemSegCollateFunction(BaseCollateFunction):
    """Plain stack of images + per-image target dicts for semantic segmentation.

    No mixup / multi-scale / copyblend: they are instance-level or would need the
    dense label resized in lockstep. `ema_restart_decay` is kept so det_solver's
    EMA refresh at the no-aug boundary behaves like every other family.
    `**unused` swallows the BatchImageCollateFunction keys that leak in from the
    gtr_base.yml collate_fn block during config merge.
    """

    def __init__(self, ema_restart_decay=0.9999, **unused) -> None:
        super().__init__()
...
class BatchImageCollateFunction(BaseCollateFunction):
    def __init__(
        self,
        mixup_prob=0.0,
        mixup_epoch=None,        # GTR legacy single epoch: mixup active while epoch < mixup_epoch
        mixup_epochs=None,       # DEIMv2-style [start, end) range; takes precedence over mixup_epoch
        stop_epoch=None,         # epoch to stop multi-scale training
        ema_restart_decay=0.9999,
        base_size=640,
        base_size_repeat=None,   # enable multi-scale training when set
        copyblend_prob=0.0,
        ...
        super().__init__()
        # Mixup epoch range: prefer the DEIMv2-style [start, end) list; otherwise fall back to
        # GTR's legacy single `mixup_epoch` (semantically epoch < mixup_epoch == [0, mixup_epoch)).
        if mixup_epochs is not None:
            self.mixup_epochs = mixup_epochs
        elif mixup_epoch is not None:
            self.mixup_epochs = [0, mixup_epoch]
        else:
            self.mixup_epochs = [0, 0]
        self.mixup_prob = mixup_prob

        # Multi-scale training scales (DEIMv2 generate_scales, inlined for single use).
    ...
    def apply_mixup(self, images, targets):
        """
        Applies Mixup (or, mutually exclusively, CopyBlend) augmentation to the batch.
        ...
        if random.random() < self.mixup_prob and self.mixup_epochs[0] <= self.epoch < self.mixup_epochs[-1]:
            ...
            for i in range(len(targets)):
                ...
                # Instance masks must stay aligned with duplicated boxes (seg + Hungarian mask cost).
                if 'masks' in targets[i] and 'masks' in shifted_targets[i]:
                    updated_targets[i]['masks'] = torch.cat(
                        [targets[i]['masks'], shifted_targets[i]['masks']], dim=0
                    )
            ...
            # CopyBlend is box-level only; skip for segmentation to avoid boxes/masks count mismatch.
            if any('masks' in t for t in targets):
                return images, targets
    ...
    def __call__(self, items):
        ...
        # Multi-scale training: resize the whole batch to a random scale until stop_epoch.
