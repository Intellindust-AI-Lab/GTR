...
class BaseConfig(object):
    ...
    def __init__(self) -> None:
        ...
        # Reload best.pth at the no-aug boundary (DEIM default). Set False to keep the
        # weights the last augmented epoch produced.
        self.no_aug_load_best: bool = True
        ...
        self.ddp_static_graph :bool = False
        self.ddp_gradient_as_bucket_view :bool = False
        self.ddp_bucket_cap_mb :int = None
        self.compile :bool = False          # opt-in torch.compile (FP32-safe; off by default)
        self.compile_mode :str = 'default'  # 'default' (DDP+dynamic friendly) or 'reduce-overhead'

        self.seed :int = None
        self.print_freq :int = None
        self.checkpoint_freq :int = 1
        self.val_freq :int = 1
        # Semantic-seg sliding-window eval (mmseg test_cfg mode='slide'). Setting it to
        # [h, w] evaluates the native-resolution image through overlapping
        # `eval_spatial_size` windows instead of squashing it; None = whole-image.
        self.slide_stride: list = None
