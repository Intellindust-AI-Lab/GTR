...
Fine-Grained Localization loss over the 6 ADR distributions.
"""
...
from .decoder import NUM_DIST
from .rbox_ops import (kld_loss_paired, rbox2distance, rbox_norm_to_rad,
                       rbox_to_hbox_xyxy)


def paired_hbox_iou_of_rbox(src_rbox: torch.Tensor, tgt_rbox: torch.Tensor) -> torch.Tensor:
...
class OBBGTRCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes_weight=None):
        ...
        if self.angle_periodic_l1:
            ...
            ang = loss_bbox[..., 4]
            loss_bbox = torch.cat([loss_bbox[..., :4], torch.minimum(ang, 1.0 - ang).unsqueeze(-1)], dim=-1)
        losses['loss_bbox'] = loss_bbox.sum() / num_boxes

        loss_kld = kld_loss_paired(rbox_norm_to_rad(src_boxes), rbox_norm_to_rad(target_boxes))
        ...
        if 'pred_corners' in outputs:
            ...
            with torch.no_grad():
                if self.fgl_targets_dn is None and 'is_dn' in outputs:
                        self.fgl_targets_dn = rbox2distance(ref_points, target_boxes,
                                                            self.reg_max, outputs['reg_scale'], outputs['up'])
                if self.fgl_targets is None and 'is_dn' not in outputs:
                        self.fgl_targets = rbox2distance(ref_points, target_boxes,
                                                         self.reg_max, outputs['reg_scale'], outputs['up'])
            ...
            if values is None:
                ...
                ious = values
            weight_targets = ious.unsqueeze(-1).repeat(1, 1, NUM_DIST).reshape(-1).detach()
            ...
            if 'loss_ddf' in self.weight_dict and 'teacher_corners' in outputs:
                ...
                if torch.equal(pred_corners, target_corners):
                    ...
                    weight_targets_local = weight_targets_local.unsqueeze(-1).repeat(1, 1, NUM_DIST).reshape(-1).detach()
                    ...
                    loss_match_local2 = (loss_match_local * neg_mask_f).sum() / neg_count
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        indices = self.matcher(outputs_without_aux, targets, group_detr=group_detr)['indices']
        self._clear_cache()
        ...
        if 'aux_outputs' in outputs:
            ...
            if self.share_matched_indices:
                ...
                for i, aux_outputs in enumerate(aux_outputs_list):
                    indices_aux = self.matcher(aux_outputs, targets, group_detr=group_detr)['indices']
                    cached_indices.append(indices_aux)
                    indices_aux_list.append(indices_aux)
                for i, aux_outputs in enumerate(outputs['enc_aux_outputs']):
                    indices_enc = self.matcher(aux_outputs, targets, group_detr=group_detr)['indices']
        ...
        if 'enc_aux_outputs' in outputs:
            ...
            for i, aux_outputs in enumerate(outputs['enc_aux_outputs']):
                for loss in self.losses:
                    ...
                    use_uni_set = self.use_uni_set and (loss == 'boxes') and group_detr == 1
